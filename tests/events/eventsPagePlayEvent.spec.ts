...
test.describe('Events Page - Events List Filter', () => {
  ...
  test('SOT-6932 | should choose first event and verify front and rear camera are have data @regression', 
        ...
        expect(await cameraEventsPage.hasData()).toBeTruthy();
        // Play first event
       await cameraEventsPage.playFirstEvent();

       // Verify front camera is playing
       await expect(cameraEventsPage.playFrontCamera).toBeVisible({ 
        timeout: TIMEOUTS.VERY_LONG
       });
       // Verify rear camera is playing
       await expect(cameraEventsPage.playRearCamera).toBeVisible({ 
        timeout: TIMEOUTS.VERY_LONG
       });
        ...
        console.log('✅ Event played and back to events list page verified');
    });
